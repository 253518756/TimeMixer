...
class CSI1000Loader(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 features='S', data_path='ETTh1.csv',
                 target='涨跌幅', scale=True, timeenc=0, freq='h', seasonal_patterns=None):
        # size [seq_len, label_len, pred_len]
        # info
        if size == None:
            self.seq_len = 24 * 4 * 4
    ...
    def __read_data__(self):
        self.scaler = StandardScaler()
        df_raw = pd.read_csv(self.root_path + 'csi1000_' + self.data_path + '_stocks_data.csv')
        ...
        self.df_raw = df_raw[border1:border2]
